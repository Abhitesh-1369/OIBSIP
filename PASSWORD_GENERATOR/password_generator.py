...
def clear():
    plength.delete(0, tkinter.END);
    platform.delete(0, tkinter.END);
    result.delete('1.0', tkinter.END);
    passrule.delete(0, tkinter.END)
...
def generate():
    ...
    try:
        if int(plength.get()) < 12:
            messagebox.showinfo(' ',
                                'For Security Reason The length of the password should be greater than equal to 12 ')
...
if __name__ == '__main__':
    ...
    # label
    tkinter.Label(screen, text='Generator a Secure Password 🥷', background=COLOR, font=('monospace', 12)).place(x=50,
                                                                                                                 y=40)
    ...
    generatebutton = tkinter.Button(screen, text='Generate', activeforeground='red', activebackground='black', bd=3,
                                    font='monospace', command=generate).place(x=250, y=250)

    clearbutton = tkinter.Button(screen, text='Clear', activeforeground='red', activebackground='black', bd=3,
                                 font='monospace', command=clear).place(x=100, y=250)  # clear

    copybutton = tkinter.Button(screen, text='Copy', activeforeground='red', activebackground='black', bd=3,
                                font='monospace', command=copyfunc).place(x=400, y=250)
